...
def load_tri_user(path, mode):
    with open(path, "r") as file:
        data = file.readlines()
        data = [tri.strip('\n').split(' ') for tri in data]
        data = [(int(tri[0]), int(tri[1]), float(tri[2])) for tri in data]
...
def get_inter_mat(args):
    ...
    os.path.exists(f"{path_root}/d2_inters.npy") and os.path.exists(f"{path_root}/d2_inters_ui.npy"):
...
def construct_user2negitems(args, train_user, user2count, user2inter, n_items):
    
    neg_ratio = args.params["neg_ratio"]
    total_neg_num = len(train_user) * neg_ratio

   
    user2negitems = defaultdict(list)
    cnt, user, next_user_cnt = 0, -1, 0
    neg_pool = []
    ...
    while cnt < total_neg_num:
        ...
        for neg_item in neg_data:
            ...
            if neg_item not in user2inter[user]:
                user2negitems[user].append(neg_item)
                neg_pool.append(neg_item)
                cnt += 1
