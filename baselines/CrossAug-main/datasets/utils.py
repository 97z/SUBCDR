...
def filter_inters_new(args, rating_inters, meta_items):
    new_inters = []
...
def filter_inters_movielens(args, rating_inters):
    new_inters = []
    # Get domain inters after the following timestamp:
    overtime_dict = {
        "Movielens": 9.76,
...
def split_sets_2(inters, user2index, item2index, neg_candidates):
    user2pos_items = collections.defaultdict(list)
    user2neg_items = collections.defaultdict(list)
    ...
    for inter in inters:
        ...
        rating = int(rating)
        if rating > 3:  
            user2pos_items[user2index[user]].append((item2index[item], rating))
        else: 
            user2neg_items[user2index[user]].append((item2index[item], rating))

    train, valid, test = [], [], []
    item_set = set(range(len(item2index)))

    for i in tqdm(range(len(user2index))):
        pos_list = random.sample(user2pos_items[i], len(user2pos_items[i]))

        
        non_inter_set = item_set - set(item for item, _ in user2pos_items[i] + user2neg_items[i])
        non_inter_list = list(non_inter_set)
        non_inter_valid = random.sample(non_inter_list, neg_candidates)
        non_inter_test = random.sample(non_inter_list, neg_candidates)
        valid.append([i, pos_list[-2][0]] + non_inter_valid) 
        test.append([i, pos_list[-1][0]] + non_inter_test) 

       
        for pos_item, rating in pos_list[:-2]:  
            train.append([i, pos_item, rating])  

        for neg_item, rating in user2neg_items[i]:  
            train.append([i, neg_item, rating]) 
