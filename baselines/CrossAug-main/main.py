...
from models import *
from utils import *
from add_model.DenoisingNet_2 import DenoisingNet

def init():
    # basic configuration
    parser = argparse.ArgumentParser()
    parser.add_argument('--domains', default='Movie-Music', help='Movie-Music/Cell-Elec/Toy-Video', type=str)
    parser.add_argument('--gpu', default=0, type=int)
    parser.add_argument('--seed', default=2024, type=int)
    parser.add_argument('--neg_valid_num', default=99, type=int)
    ...
    parser.add_argument('--al', default=True, type=bool)

    parser.add_argument('--al_para', default=0.5, type=float)
    parser.add_argument('--checkpoint_dir', default='/root/autodl-tmp/Crossaug/checkpoint', type=str)
    parser.add_argument('--negdict', default=False, type=bool)
    parser.add_argument('--load_check_train', default=False, type=bool)

    args = parser.parse_args()

    args.dataset = "Amazon"
    #assert args.domains in ["Movie-Music", "Cell-Elec"]
    args.approach = "CrossAug" # or AbtCDR
...
def load_models(args, config):
    ...
    if compile:
        ...
        args.model = getattr(sys.modules[__name__], args.approach)(args).to(args.device)
...
def train(args):
    ...
    epoch = 0
    evaluate(args, epoch, "valid")
    for i in range(epoch, n_epoch):
        args.model.train()
        ...
        if args.negdict:
            d1_neg, d1_neg_dict = construct_user2negitems(args, d1_train[:, 0], d1_user2count, d1_user2item, args.d1["n_items"])
            d2_neg, d2_neg_dict = construct_user2negitems(args, d2_train[:, 0], d2_user2count, d2_user2item, args.d2["n_items"])
        ...
        if args.graph_denoise:
            temperature = max(0.05, 2.0 * pow(0.98, i))
        ...
        if (i + 1) % 10 == 0:
            checkpoint_path = os.path.join(args.checkpoint_dir, f"checkpoint_epoch_{i + 1}.pth")
            torch.save({
                'epoch': i + 1,
                'model_state_dict': args.model.state_dict(),
                'optimizer_state_dict': args.optim.state_dict(),
                'loss': loss.item(),
            }, checkpoint_path)
...
def evaluate(args, epoch=None, mode="test", eval_size=128):
    args.model.eval()
...
def load_checkpoint(checkpoint_path, model_class, args):
    ...
    optimizer = getattr(torch.optim, args.params['optim'])(model.parameters(), lr=args.lr)
    optimizer.load_state_dict(optimizer_state_dict)

    print(f"Checkpoint loaded from {checkpoint_path}, trained up to epoch {epoch}.")
...
if __name__ == '__main__':
    ...
    train(args)
    evaluate(args)
    checkpoint_path = os.path.join(args.checkpoint_dir, f"last_model.pth")
    torch.save(args.model.state_dict(), checkpoint_path)
