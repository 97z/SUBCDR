...
class CrossAug(nn.Module):
    ...
    def graph_transfer_abtcdr(self, d1_emb, d2_emb):
        d1_graph = self.graph_layer(self.d1_norm_adj, d1_emb)
        d2_graph = self.graph_layer(self.d2_norm_adj, d2_emb)
        d1_trans, d2_trans = self.inter_embedding(d1_graph, d2_graph)
        d1_norm = F.normalize(d1_trans, 2, -1)
        d2_norm = F.normalize(d2_trans, 2, -1)
    ...
    def inter_embedding(self, source_all_embeddings, target_all_embeddings):
        ...
        adj_s = normalized_adj_single_(adj_s + torch.eye(self.n_shared_users).cuda())  # 修改归一化函数
        ...
        source_user_full[:self.n_shared_users] = src_user  # 替换前n行
    ...
    def forward(self):
        ...
        for i in range(self.n_layers):
            if self.abtcdr:
                d1_emb, d2_emb = self.graph_transfer_abtcdr(d1_emb, d2_emb)
            else:
    ...
    def calculate_loss(self, inter_d1, neg_d1_item, inter_d2, neg_d2_item):
        ...
        if self.align_lambda:
            align_loss = align_loss + self.al_para * self.get_align_loss(d1_user_emb[:self.n_shared_users],
                                                                         d2_user_emb[:self.n_shared_users])
            align_loss = align_loss + self.al_para * self.get_align_loss(d1_items, d2_items)
        ...
        return total_loss
    def get_items_from_negdict(self, negdict, ids, num_samples=1):
        ...
        for user_id in ids:
            ...
            neg_items = negdict.get(user_id, [])

            
            if neg_items:
                sampled_neg = np.random.choice(neg_items, size=num_samples, replace=False)
                negative_samples.append(sampled_neg[0])  
            else:
