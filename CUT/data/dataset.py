...
from recbole.utils import ensure_dir #add for saving dataset
...
class CrossDomainSingleDataset(Dataset):
    ...
    def get_sparse_matrix(self, user_num, item_num, form='coo', value_field=None):
        ...
        user_degrees = np.array(mat.sum(axis=1)).flatten()
        if form == 'coo':
            return mat, user_degrees
        elif form == 'csr':
            return mat.tocsr(), user_degrees
        else:
...
class CrossDomainDataset:
    ...
    def build(self):
        ...
        See :class:`~SUBCDR.config.eval_setting.EvalSetting` for details.
        Returns:
        ...
        if not source_split_flag:
            ...
            source_domain_train_dataset, source_domain_valid_dataset, source_domain_test_dataset = self.source_domain_dataset.build()

            return [source_domain_train_dataset, source_domain_valid_dataset, source_domain_test_dataset, target_domain_train_dataset,
                    target_domain_valid_dataset, target_domain_test_dataset]

    
    def merge_adj_matrices(self, d1_adj, d2_adj):
       
        row = np.concatenate([d1_adj.row, d2_adj.row])
        col = np.concatenate([d1_adj.col, d2_adj.col])
        
        data = np.ones_like(row)
        combined_adj = coo_matrix((data, (row, col)), shape=(self.num_total_user, self.num_total_item))
        return combined_adj
    def merge_adj_tensor_matrices(self, d1_adj, d2_adj):
        
        assert d1_adj.is_sparse, "d1_adj should be a sparse tensor."
        assert d2_adj.is_sparse, "d2_adj should be a sparse tensor."

        
        d1_row, d1_col = d1_adj._indices()
        d2_row, d2_col = d2_adj._indices()

       
        row = torch.cat([d1_row, d2_row], dim=0)
        col = torch.cat([d1_col, d2_col], dim=0)

        
        data = torch.ones_like(row, dtype=torch.float64)

        
        combined_adj = torch.sparse_coo_tensor(torch.stack([row, col]), data,
                                               size=(self.num_total_user+self.num_total_item, self.num_total_user+self.num_total_item))
        return combined_adj
    
    def calculate_user_degree(self, adj_matrix):
        
        user_degree = adj_matrix.getnnz(axis=1)
        return user_degree
    ...
    def process_user_file(self, file_path, src_user_remap_dict, tgt_user_remap_dict):
        row_indices = []
        col_indices = []
        value = []
        aspect = []
        for idx, line in enumerate(open(file_path, 'r').readlines()):
            data = line.strip().split()
            origintgt_user_id = data[0]
            originsrc_user_id = data[1]

            
            mappedtgt_user_id = tgt_user_remap_dict[origintgt_user_id]
            mappedsrc_user_id = src_user_remap_dict[originsrc_user_id]
            
            row_indices.append(mappedtgt_user_id)
            col_indices.append(mappedsrc_user_id)
            value.append(1)  
            aspect.append(data[2])
        ...
        try:
            _ = float(aspect[0])
            aspect = ['similary'] * len(aspect)
        except:
            print('hetero graph')

        return value, row_indices, col_indices, np.array(aspect)
    ...
    def save(self): #add from other classes of datasets
    ...
    def get_history_matrix(self, row, value_field=None, max_history_len=None): #add from recbole dataset.py
        #Here it returns the history from both domains
        """Get dense matrix describe user/item's history interaction records.
        ...
        for domain in ['source', 'target']:
            if domain == 'source':
                domain_dataset = self.source_domain_dataset
            else:
                domain_dataset = self.target_domain_dataset
            inter_feat = copy.deepcopy(domain_dataset.inter_feat)
            inter_feat.shuffle()
            user_ids, item_ids = (
                inter_feat[domain_dataset.uid_field].numpy(),
                inter_feat[domain_dataset.iid_field].numpy(),
            )
            if value_field is None:
            ...
            user_id_domains.append(user_ids)
            item_id_domains.append(item_ids)
            value_domains.append(values)

        user_ids = np.concatenate(user_id_domains)
        item_ids = np.concatenate(item_id_domains)
        values = np.concatenate(value_domains)

        perm_order = np.random.permutation(len(user_ids))
        user_ids = user_ids[perm_order]
        item_ids = item_ids[perm_order]
        values = values[perm_order]

        if row == "user":
            row_num, max_col_num = self.num_total_user, self.num_total_item
            row_ids, col_ids = user_ids, item_ids
        else:
            row_num, max_col_num = self.num_total_item, self.num_total_user
            row_ids, col_ids = item_ids, user_ids
    ...
    def history_item_matrix(self, value_field=None, domain='total', max_history_len=None):
        """Get dense matrix describe item's history interaction records.
        ...
        """
        if domain == 'total':
            return self.get_history_matrix(row='user', value_field=value_field, max_history_len=max_history_len)
        elif domain == 'source':
            return self.source_domain_dataset.get_history_matrix(self.num_total_user, self.num_total_item,
    ...
    def num(self, field): #add from recbole dataset class
        ...
        """
        if field not in ['total_user_id', 'total_item_id']:
            raise NotImplementedError(f"Field [{field}] should be total_user_id or total_item_id.")

        if field == 'total_user_id':
            return self.num_total_user
        elif field == 'total_item_id':
            return self.num_total_item
