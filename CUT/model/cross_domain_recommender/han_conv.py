...
class HANConv(MessagePassing):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.k_lin = nn.Linear(out_channels, out_channels, dtype=torch.float64)
        self.q = nn.Parameter(torch.empty(1, out_channels, dtype=torch.float64))

        self.proj = nn.ModuleDict()
        for node_type, in_channels in self.in_channels.items():
            self.proj[node_type] = Linear(in_channels, out_channels)
            # Convert the weights to float64
            self.proj[node_type].weight.data = self.proj[node_type].weight.data.to(torch.float64)
            self.proj[node_type].bias.data = self.proj[node_type].bias.data.to(torch.float64)
        ...
        for edge_type in metadata[1]:
            ...
            self.lin_src[edge_type] = nn.Parameter(torch.empty(1, heads, dim, dtype=torch.float64))
            self.lin_dst[edge_type] = nn.Parameter(torch.empty(1, heads, dim, dtype=torch.float64))
