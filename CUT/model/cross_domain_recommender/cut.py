...
class CUT(CrossDomainRecommender):
    ...
    def neg_sort_predict(self, interaction,test_neg=None,valid_neg=None):
        '''if self.single_model_config['name'] == 'LightGCN':
            if self.restore_user_e is not None or self.restore_item_e is not None:
                self.restore_user_e, self.restore_item_e = None, None'''
        aie = self.full_sort_get_all_item_weight()
        ...
        if self.single_model_config['name'] in ['MF','LightGCN']:
            ...
            if neg==1:
                if valid_neg is not None:
                    ...
                    batch_user_ids = interaction[self.TARGET_USER_ID]  
                    ...
                    batch_pos_item_emb = all_item_e[batch_pos_item_indices]  # 形状 (user_num, embedding_dim)
                    batch_neg_item_emb = all_item_e[
                        torch.tensor(batch_neg_item_indices).view(-1)
                    ].view(len(batch_user_ids), 99, all_item_e.size(1))  # 形状 (user_num, 99, embedding_dim)
                    ...
                    score_valid = torch.bmm(user_e.unsqueeze(1), item_emb.transpose(1, 2)).squeeze(1)  
                if test_neg is not None:
                    ...
                    score_test = torch.bmm(user_e.unsqueeze(1), item_emb.transpose(1, 2)).squeeze(1)  # 形状 (user_num, 100)

                    
                return self.sigmoid(score_valid),self.sigmoid(score_test)
            else:
                score = torch.matmul(user_e, all_item_e.transpose(0, 1))
        elif self.single_model_config['name'] in ['SimpleX']:
            user_e = F.normalize(user_e, dim=1)
