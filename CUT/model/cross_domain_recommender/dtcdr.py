...
class DTCDR(CrossDomainRecommender):
    ...
    def calculate_loss_base_cut(self, interaction):
    ...
    def neg_sort_predict(self, inter, test_neg=None, valid_neg=None):
        ...
        user_source_e = self.target_user_embedding(user_ids)
        user_target_e = self.source_user_embedding(user_ids)
        user_e = torch.maximum(user_source_e,user_target_e)

        if valid_neg is not None:
            
            batch_pos_item_indices = []
            batch_neg_item_indices = []
            for user_id in user_ids:
                user_id_int = user_id.item()
                
                pos_item_id = valid_neg[user_id_int]['positive']
                neg_item_ids = valid_neg[user_id_int]['negative']
                batch_pos_item_indices.append(pos_item_id)
                batch_neg_item_indices.append(neg_item_ids)

            
            batch_pos_item_indices_tensor = torch.tensor(batch_pos_item_indices, device=user_e.device)
            batch_pos_item_emb_s = self.source_item_embedding(
                batch_pos_item_indices_tensor) 
            batch_pos_item_emb_t = self.target_item_embedding(batch_pos_item_indices_tensor)

            batch_neg_item_indices_tensor = torch.tensor(batch_neg_item_indices, device=user_e.device).view(-1)
            batch_neg_item_emb_s = self.source_item_embedding(batch_neg_item_indices_tensor).view(len(user_ids), 99,
                                                                                                  self.source_item_embedding.weight.size(
                                                                                                      1))
            batch_neg_item_emb_t = self.target_item_embedding(batch_neg_item_indices_tensor).view(len(user_ids), 99,
                                                                                                  self.target_item_embedding.weight.size(
                                                                                                      1))  

            batch_pos_item_emb = torch.maximum(batch_pos_item_emb_s,batch_pos_item_emb_t)
            batch_neg_item_emb = torch.maximum(batch_neg_item_emb_s,batch_neg_item_emb_t)
            # 拼接正样本和负样本嵌入
            item_e = torch.cat(
                [batch_pos_item_emb.unsqueeze(1), batch_neg_item_emb], dim=1
            )  
            user_e = user_e.unsqueeze(1).expand(-1, item_e.size(1), -1)
            if domain == 'src':
                score_valid = self.source_sigmoid(self.source_predict_layer(self.source_mlp_layers(torch.cat((user_e, item_e), -1))))
            else:
                score_valid = self.target_sigmoid(self.target_predict_layer(self.target_mlp_layers(torch.cat((user_e, item_e), -1))))
        if test_neg is not None:
            ...
            for user_id in user_ids:
                ...
                pos_item_id = test_neg[user_id_int]['positive']
                neg_item_ids = test_neg[user_id_int]['negative']
                batch_pos_item_indices.append(pos_item_id)
                batch_neg_item_indices.append(neg_item_ids)
            ...
            if domain == 'src':
                score_test = self.source_sigmoid(
                    self.source_predict_layer(self.source_mlp_layers(torch.cat((user_e, item_e), -1))))
            else:
                score_test = self.target_sigmoid(
                    self.target_predict_layer(self.target_mlp_layers(torch.cat((user_e, item_e), -1))))

        score_valid = score_valid.squeeze(-1) if score_valid is not None else None
        score_test = score_test.squeeze(-1) if score_test is not None else None
        return score_test, score_valid
